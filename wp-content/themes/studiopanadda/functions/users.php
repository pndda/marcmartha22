<?php 
/*
 * Create studiopanadda User role
 */
...
// Add new role
add_role('studiopanadda', __('Studio Panadda User', 'studiopanadda'), $capebilities);

// Set new user role to default
add_filter('pre_option_default_role', 'stdp_set_default_user_role');

function stdp_set_default_user_role(){
    return 'studiopanadda';
};
...
/*
 * Add Custom link to site-menus to sidebar - only for studiopanadda
 */
...
function add_menu_url() {
    ...
    if (in_array('studiopanadda', $roles_arr)) {
        add_menu_page('Menus', 'Menus', 'read', admin_url() . '/nav-menus.php', '', 'dashicons-admin-links', 75);
    }
}

/*
 * Limit backend for studiopanadda User
 */

add_action('admin_init', 'remove_menus');
function remove_menus() {
    $user = wp_get_current_user();
    if(in_array('studiopanadda', $user->roles)) {
        $restrictions = array(
...
/**
 * WHEN stdp-USER gets access to user management, prevent him from editing/adding/removing existing/new admins!
 */

// Helper function
function stdp_role_is_admin() {
...
function stdp_edit_users_list( $args ){
    if( !stdp_role_is_admin() ){
...
add_filter('users_list_table_query_args', 'stdp_edit_users_list');

//Removing admin view from list/will be empty anyways
function stdp_edit_users_views( $views ) {
    if( !stdp_role_is_admin() ){
...
add_filter('views_users', 'stdp_edit_users_views');

// Remove administrator from all user-roles lists when user is not admin
function stdp_edit_user_roles( $roles ) {
    if( !stdp_role_is_admin() ){
...
add_filter('editable_roles', 'stdp_edit_user_roles');

// Prevent extra-user of viewing edit-user.php for admin-id's
function stdp_check_user_has_right_to_edit_user() {
    global $pagenow;
    ...
    if( $pagenow == 'user-edit.php' ){
        ...
        if( $user_id != $edit_user_id) { //If not your own page!
            ...
            if( !stdp_role_is_admin() && in_array( 'administrator', $user_roles ) ){
                wp_die( __( 'Sorry, you are not allowed to edit this user.' ) );
            }
        }
    }
}
add_action('wp_loaded', 'stdp_check_user_has_right_to_edit_user');
